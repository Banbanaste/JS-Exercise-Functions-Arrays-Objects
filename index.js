...
function getCarInfoById(inventory, id) {
  /* code here */
  /* im cheating here a bit, you could loop through it */
...
function getModelYears(inventory) {
  ...
  inventory.forEach(car => {
    if (modelYears.indexOf(car.car_year) == -1) {
  ...
  */
  ...
  for (let index = 0; index < inventory.length; index++) {
    ...
    modelYears.push(element.car_year);
  }
...
function getGermanCars(inventory) {
  /* code here */
  const euro = [`Audi`, `Mercedes-Benz`, `Volkswagen`, `BMW`];
  let euroOnlyInventory = [];
  for (let index = 0; index < inventory.length; index++) {
    const element = inventory[index];
    if (euro.indexOf(element.car_make) !== -1) {
      euroOnlyInventory.push(element);
    }
  }
  return euroOnlyInventory;
}
